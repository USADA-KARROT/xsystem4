...
- ❌ SceneLogo 背景仍然黑色：Base parts 900002 alpha 正確遞增，PE_BuildConstructionProcess 有被呼叫（ops=1），但可能只有 CREATE_PIXEL_ONLY 沒有 FILL_ALPHA_COLOR（需要 2 ops 才能產生白色矩形）。Activity .ex 檔案載入/解析可能不完整。

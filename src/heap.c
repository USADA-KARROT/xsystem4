...
void heap_set_page(int slot, struct page *page)
{
	...
	if (unlikely(slot == 0 && page && page->type != GLOBAL_PAGE)) {
		WARNING("heap_set_page: refusing to overwrite global page (slot 0) with type=%d ip=0x%lX fno=%d",
			page->type, (unsigned long)instr_ptr,
			call_stack_ptr > 0 ? call_stack[call_stack_ptr-1].fno : -1);
		return;
	}
	// Diagnostic: catch when any page with ref > 0 is set to NULL
	if (unlikely(!page && slot > 0 && heap[slot].page != NULL)) {
		static int null_page_warn = 0;
		if (null_page_warn++ < 20) {
			WARNING("heap_set_page: slot %d (type=%d index=%d nr_vars=%d ref=%d) -> NULL ip=0x%lX fno=%d",
				slot, heap[slot].page->type, heap[slot].page->index, heap[slot].page->nr_vars,
				heap[slot].ref, (unsigned long)instr_ptr,
				call_stack_ptr > 0 ? call_stack[call_stack_ptr-1].fno : -1);
		}
	}
	heap[slot].page = page;

...
void PE_EndMotion(void)
{
	...
	// Signal WaitForClick to exit so Motion::Join can re-check IsEnd.
	// The script-level Motion::Join loop calls WaitForClick to yield
	// one frame; WaitForClick only exits when global[2] != 0 (or
	// key/click input).  PE_EndMotion fires during WaitForClick's
	// AFL_View_Update → SystemService.UpdateView → PE_UpdateMotionTime
	// chain, so setting global[2]=1 here lets WaitForClick return.
